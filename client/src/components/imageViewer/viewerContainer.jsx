...
export default class ViewerContainer extends Component {
    ...
    setMode (mode) {
        // mode is one 'Proj', 'Slice', 'Volume'
        if (mode==='Volume') {
            const tempCanvas = document.createElement('canvas');
            const clientHasWebgl2 = tempCanvas.getContext('webgl2')!==null;
            tempCanvas.remove();
            if (!clientHasWebgl2) {
                this.setState({showWebglWarning: true});
                return;
            }
        }
        this.setState({mode});
    ...
    render () {
        ...
        // URLs to the raw TIFFs on S3
        const rawTiffUrl = kind => {
            const sep = '_';
            const urlRoot = 'https://czb-opencell.s3.amazonaws.com/microscopy/raw';
            const dirname = [this.props.targetName, this.props.ensgId].join(sep);
            const filenameParts = [
                `OC-FOV`,
                dirname,
                `CID${this.props.cellLineId?.toString().padStart(6, '0')}`,
                `FID${this.props.fovId?.toString().padStart(8, '0')}`,
                `${kind}.tif`,
            ];
            return `${urlRoot}/${dirname}/${filenameParts.join(sep)}`;
        };

        const lowGfpWarning = (
            <div className='low-gfp-warning-container'>
                <Popover>
                    <Callout title='Low signal' intent='warning' targetClassName='f6'/>
                    {popoverContents.lowGfpWarning}
                </Popover>
            </div>
        );
        ...
        const webglWarning = (
            <Dialog
                className=''
                icon='warning-sign'
                isOpen={this.state.showWebglWarning}
                onClose={() => this.setState({showWebglWarning: false})}
                title='The 3D viewer requires WebGL2'
            >
                <div className={Classes.DIALOG_BODY}>
                    <p>
                    Oops! Unfortunately, it looks like your browser does not support WebGL2, 
                    which is required for the 3D image viewer to work correctly. 
                    Please consider <a href='https://get.webgl.org/webgl2/' target='_blank'>
                        upgrading your browser
                    </a>. 
                    </p>
                    <p>
                    Note that Safari does not yet support WebGL2; if you are using Safari,
                    please switch to a recent version of Firefox, Opera, or Chrome to use the 3D viewer.
                    </p> 
                </div>
            </Dialog>
        );
        ...
        return (
                ...
                <div className='pl2'>
                    ...
                    <div className='w-100 flex flex-row pt2'>
                        ...
                        <div className='flex'>
                            <div className=''>
                                <div className='flex items-center'>
                                    <div className='pr1 button-group-label'>Download</div>
                                    <Popover>
                                        <Icon icon='info-sign' iconSize={10} color="#bbb"/>
                                        {popoverContents.microscopyDownloadRaw}
                                    </Popover>
                                </div>
                                <div className='flex'>
                                    <a href={rawTiffUrl('proj')}><SimpleButton text='2D'/></a>
                                    <a href={rawTiffUrl('stack')}><SimpleButton text='3D'/></a>
                                </div>
                            </div>
                        </div>
            ...
            <div className='pt2 viewer-and-overlay-container'>
                <div className='viewer-container'>{viewer}</div>
                {this.props.isLowGfp ? lowGfpWarning : null}
            </div>
            ...
            <div className='flex flex-wrap w-100 pt2 pb2'>
                ...
                {/* z-index slider */}
                <div
                    className='w-100 flex flex-0-0-auto pr3'
                    style={{'visibility': this.state.mode!=='Slice' ? 'hidden': 'visible'}}
                >
                    <div className='w-30'>
                        <span>Confocal slice at </span>
                        <span className='b'>
                            {`${zIndexToMicrons(this.state.zIndex)}`}<span>&micro;m</span>
                        </span>
                    </div>
                    <div className='w-70'>
                        <Tooltip
                ...
                <div className='flex-0-0-auto w-50 pr4'>
                    <div className='w-100 display-settings-label'>Nucleus image settings</div>
                    <div className='pt1 flex justify-between'>
                        <div className='viewer-slider-label'>Intensity range</div>
                        <div className='viewer-slider-value'>
                            {`${this.state.min405}% - ${this.state.max405}%`}
                        </div>
                    </div>
                    ...
                    <div className='flex justify-between'>
                        <div className='viewer-slider-label'>Gamma</div>
                        <div className='viewer-slider-value'>{`${this.state.gamma405.toFixed(2)}`}</div>
                    </div>
                    <Slider
                        min={0.5}
                        max={1.5}
                ...
                <div className='flex-0-0-auto w-50 pl1 pr3'>
                    ...
                    <div className='w-100 display-settings-label'>Target image settings</div>
                    <div className='pt1 flex justify-between'>
                        <div className='viewer-slider-label'>Intensity range</div>
                        <div className='viewer-slider-value'>
                            {`${this.state.min488}% - ${this.state.max488}%`}
                        </div>
                    </div>
                    ...
                    <div className='flex justify-between'>
                        <div className='viewer-slider-label'>Gamma</div>
                        <div className='viewer-slider-value'>{`${this.state.gamma488.toFixed(2)}`}</div>
                    </div>
                    <Slider
                        min={0.5}
                        max={1.5}
