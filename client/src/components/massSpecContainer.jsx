...
function Tabs (props) {
    ...
    const ActiveTab = children.filter(child => child.props.id===activeTabId)[0];
    return (
        <>
...
export default function MassSpecContainer (props) {
    ...
    if (props.layout==='tabs') content = (
        <Tabs activeTabId='network'>
            <Tab 
                id='network' 
                title='Show interaction network'
                component={network}
                popoverContent={popoverContents.interactionNetworkHeader}
            />
            {idType==='pulldown' ? scatterplotTab : null}
            <Tab 
                id='table' 
                title='Show list of interactors' 
                component={table}
